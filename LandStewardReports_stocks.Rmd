...
```
```{r, SOC stats}
#SOC STOCKS INTERPRETATION STATISICS
```
...
```
```{r, eval = any(!stocks_df$Stocks_Indicator %in% c("org_c_stocks", "inorg_c_stocks", "all_stocks")),fig.cap="Average carbon stocks for all measured pools. Organic carbon stocks are repeated as above to show comparison to other carbon pools measured. The values shown for all measured carbon pools combined is also shown."}
