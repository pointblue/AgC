...
if(length(unique(PointLevel$timepoint))>1){
  ...
  time_change_bd <- contrast(emm, method = setNames(list(c(-1, 1)),paste0(tp_last, " - ", tp_first))) %>%
    ...
    mutate(
      text = case_when(
        p.value < 0.05 & estimate > 0 ~ "Bulk density in the treated plot **increased** between the first and most recent monitoring timepoint. Though it may seem the practice is improving soil structure, it may also be a result of other environmental or management changes across years. A longer monitoring timeframe will strengthen our confidence in this result.",
        p.value < 0.05 & estimate < 0 ~ "Bulk density in the treated plot **decreased** between the first and most recent monitoring timepoint. Without a control field, we are unable to tell whether this is due to environmental or management changes across years. A longer monitoring timeframe will strengthen our confidence in this result.",
        TRUE                          ~ "Bulk density **did not change** between the two monitoring timepoints. Management either protected existing soil structure, rather than improving it, or high soil variability and/or a short timeframe made changes hard to detect."
      )
    )%>%
    pull(text) %>%          # get vector of strings
    paste(collapse = " ")
...
treatment_contrast_bd<-contrast(emm, method = setNames(list(c(-1, 1)), "T - C")) %>%
  ...
  mutate(
    text = case_when(
      ...
      TRUE ~ "There is **no significant difference** between bulk density in the treated and control site at this time. This means that the sites are well-matched."
    )
  )%>%
  pull(text) %>%          # get vector of strings
  paste(collapse = " ")
...
if(length(unique(PointLevel$timepoint))>1 && length(unique(PointLevel$plot_type))>1){
  ...
  if (length(unique(time_contrasts_bd$condition)) >1){
    ...
    if (all(time_contrasts_bd[time_contrasts_bd$plot_type == "T", ]$condition == "increased") & all(time_contrasts_bd[time_contrasts_bd$plot_type == "C", ]$condition == "decreased")) { 
      baci_bd<-"While bulk density at your **control site is decreasing**, bulk density at the **treatment site is increasing**. This indicates an unexpected negative impact of the conservation practice or inherent differences between the treatment and control plots. Consider contacting the Point Blue Soils Team for further interpretation."
    }
    ...
    if (all(time_contrasts_bd[time_contrasts_bd$plot_type == "T", ]$condition == "decreased") & all(time_contrasts_bd[time_contrasts_bd$plot_type == "C", ]$condition == "increased")) {
      baci_bd<-"While bulk density in the **treatment plot is decreasing**, bulk density in the **control plot is increasing**. This indicates the management intervention is effectively protecting and enhancing soil structure, as expected. "
    }
