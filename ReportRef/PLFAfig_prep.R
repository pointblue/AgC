...
plfa_labels <- c(fung_bio = "Fungal Biomass", bact_bio = "Bacterial Biomass")

#Begin graph
PLFAfig<-ggplot(PLFA_df, aes(x = timepoint, y = biomass))+
...
#  ) +
  ...
  facet_grid(
    ...
    scales="free_x",
    labeller = labeller(
      PLFA = plfa_labels,           # use the named vector for PLFA
    )
  )
