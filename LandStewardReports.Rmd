...
asis_output("### Soil organic carbon percents\\n") #CONDITIONAL HEADER
```
...
if (length(setdiff(intersect(proj.indicators.SSC, stocks.inds), c('poc_stocks', 'maoc_stocks')))>1){ #if the number of stocks indicators in this project (minus poc and maoc) are greater than one, create a summed all stocks column
  ...
  proj.indicators.SSC<-c(proj.indicators.SSC, "all_stocks") #CHECK! I may have added this to the wrong vector previously. Need to see how this is used 
}

}
```

```{r, formatting stocks data, eval = FALSE}
#CHECK! Here's the eval statement if you want to re-activate this chunk: "bulk_density" %in% proj.indicators.SSC && params$stocks
...
```
```{r results='asis', eval = any(!stocks_df$Stocks_Indicator %in% c("org_c_stocks", "inorg_c_stocks", "all_stocks"))}
...
```
```{r, eval = FALSE,fig.cap="Average carbon stocks for all measured pools. Organic carbon stocks are repeated as above to show comparison to other carbon pools measured. The values shown for all measured carbon pools combined is also shown."}
#CHECK! Here's the eval statement if you want to reactive this chunk any(!stocks_df$Stocks_Indicator %in% c("org_c_stocks", "inorg_c_stocks", "all_stocks"))
...
if ("Tx" %in% proj.indicators & (length(unique(PointLevel$timepoint)) > 1)) {
  cat("Note that soil texture is considered an inherent soil property -- we don't expect these values to change due to management intervention. For this reason, texture is **generally only measured at the first monitoring timepoint** to help contextualize results going forward.")
}
...
#CHECK super close but theres something wrong with SIC and NaN to NA didn't work
...
```{r, eval= unique(PointLevel$timepoints)>1}
#Chunk only runs if there's more than one timepoint
