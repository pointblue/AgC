...
ggplot(df_long, aes(x = plot_type, y = Tons.Acre, color = plot_type)) +

  facet_wrap(~ Stocks_Indicator) +
  theme_minimal() +
  
  # jittered sampling points
  geom_point(aes(color = "Sampling point values"), alpha = .5, size = 3, position = position_jitterdodge(jitter.width = 1, jitter.height = 0.1))+
  ...
  #Color scale for both plot_type and sampling points
  scale_color_manual(
    values = c(
      "C" = cbbPalette[1],
      "T" = cbbPalette[2],
    ...
    breaks = c("C", "T", "Sampling point values"),
    labels = c(str_wrap("Control site average with 95% confidence interval", width=21), 
               str_wrap("Treated site average with 95% confidence interval", width=21), 
  ...
  facet_manual(
    ~Stocks_Indicator,
    design = design,
    labeller = as_labeller(Var.Labels, default = label_wrap_gen(20))
  ) +
