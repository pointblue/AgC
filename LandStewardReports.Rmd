...
timepoint_sentence <- timepoint_dates %>%
  arrange(timepoint) %>%  # optional: ensure proper order
...
#INSERT BD STATS INTERPRETATION HERE
...
ggplot(bd_df, aes(x = timepoint, y = bulk_density))+
  ...
  stat_summary(
    ...
    bg.r = 0.15,  #thickness of outline
    aes(
      label = sprintf("%.1f", after_stat(y)), #change decimal places printed by altering the digit
      group = timepoint
      ), 
    color = "#FFD700", #text color
    hjust = 1.2,
  ...
  scale_x_discrete(labels = c(
    ...
    "Reference (2010)" = "Reference (2010)"
      ...
      stat_summary(
        ...
        bg.color = "black", #text outline color
        bg.r = 0.15,  #thickness of outline
        aes(label = sprintf("%.1f", after_stat(y))), #change decimal places printed by altering the digit
        color = "#FFD700", #text color
        hjust = 1.2,
        vjust = -1.2,
        fontface = "bold",
...
for (tp in timepoints) {
  header_above <- c(header_above, setNames(length(plot_types), tp))
}
...
```{r leaflet_maps_by_timepoint, results='asis', echo=FALSE, message=FALSE, warning=FALSE}
library(htmltools)  #for tagList(), needed if we're going to list multiple maps. 
...
if ("bulk_density" %in% proj.indicators.SSC.stocks && params$stocks==TRUE) { #combine into one vector 
  proj.indicators.nonbiomass<-proj.indicators.SSC.stocks[!proj.indicators.SSC.stocks %in% c("AHB", "HRB", "AWB", "WRB")] #CHECK! irrelevant for now but will be important later...get the rest of the project indicators that can be reported not as stocks...this matters for depth increment stuff
...
# Collect output in a list of HTML tags
map_outputs <- list()

#helper function from chatgpt is a workaround so that the popup tables can take the exact format i want
...
for (tp in unique(PointLevel$timepoint)) {

  # --- Header for the timepoint section ---
  header_html <- tags$h4(paste("Timepoint:", tp))
  ...
  popupcols <- setdiff(names(df_forpopup), c("sample_id", "geometry", "plot_type"))
  df_forpopup$popup_html <- apply(df_forpopup, 1, function(r)
    build_popup_html(r, popupcols)
  )
  ...
  for (i in seq_along(dynamicmap.inds)) {
    ...
    domainz <- if (all(is.na(DataMap.proj[[indicator]]))) c(0, 1) else DataMap.proj[[indicator]]
  ...
  # Store header and map as a tag pair
  map_outputs <- append(map_outputs, list(header_html, DynamicMap))
}

# Render all maps and headers together
browsable(tagList(map_outputs))

```

```{r, eval= FALSE}
#Chunk only runs if there's more than one timepoint: unique(PointLevel$timepoints)>1

#SETUP BASELINE AND CHANGEG DYNAMIC MAP

#subset dataframes for the first and last timepoint sampled
...
#For lastT, replace relevant columns with firstT-LastT to get a change value
