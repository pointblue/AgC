...
title: |
  ...
  Land Steward Report
author: "Avalon Cook, Lisa Eash, Greg Richardson, Ryan DiGaudio, Erika Foster, Chloe Latt, and Kees Hood"
...
if (identical(allpoints.list, proj.indicator.table)==FALSE){ #only proceed if some indicators were not measured at every point
  ...
  if (all(apply(!is.na(PointLevel[subset.inds]), 1, function(x) all(x) | all(!x)))){ #returns TRUE if all subset indicators were measured at the same sampling points
    ...
    cat(paste0("The following indicators were measured at points ", format_list(substr(subset_points, 6, 7)), ": ", format_list(subset.inds.name),"<br><br>")) #CHECK! change substring values when point codes are standardized
  }
