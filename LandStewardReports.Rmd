...
```
``` {r, results='asis', eval = unique(stocks_df$Stocks_Indicator)>1, include=FALSE}
asis_output(
...
```
```{r, Stocks figure for all other pools, fig.cap="", include=FALSE}
...
stocks_df_forallpools$Stocks_Indicator <- factor(stocks_df_forallpools$Stocks_Indicator, levels = c("org_c_stocks", "inorg_c_stocks", "all_stocks"))
...
indicator_labels <- c(
  ...
  "all_stocks" = "All measured carbon pools"
)
...
```
### All soil properties
The following table summarizes average values across the study area for all soil metrics included in your project. 
