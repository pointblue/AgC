...
stocks_df_forallpools <- stocks_df_forallpools %>%
  mutate(
    # Replace NA with a string so interaction works
    timepoint_label = ifelse(is.na(timepoint), "Reference", timepoint),
    # Create interaction for x-axis
    plot_time = interaction(plot_type, timepoint_label, sep = " | ")
  )%>%  mutate(
    plot_time = fct_reorder(plot_time, as.numeric(factor(plot_type)))
  )
...
# --- clean up NaN values ---
Means.Pivot[is.nan(as.matrix(Means.Pivot))] <- NA
...
value_columns <- as.vector(outer(timepoints, plot_types, paste, sep = " | "))

# Reorder columns to match that structure
Means.Pivot <- Means.Pivot %>%
  select(Acronym, all_of(value_columns), Units)

# Lower header (flat_colnames) — show plot types only
