...
if ((length(unique(PointLevel$timepoint)) == 1) && (unique(PointLevel$timepoint) == "T0")){
  cat("\\n Note that results shown in this report are for the **baseline timepoint only**. This means that these data are a starting point for assessing conditions prior to management change and no impact from conservation practices is shown.")
}
```
...
```
#### PLFA Results {.hidden}
```{r results='asis', eval = (all_of("fung_bio", "bact_bio") %in% proj.indicators.SSC)}
#This will only evaluate if there is no bulk density or if the user opted not to show stocks in the report
asis_output("### Phospholipid Fatty Acid Results\\n") #CONDITIONAL HEADER
```
PLFA (phospholipid fatty acid) analysis measures the microbial community in soil by analyzing signature molecules from bacterial, fungal, and other microbes. It provides insights into community composition and soil biological activity, which are key indicators of soil health.
```{r eval = !("bulk_density" %in% proj.indicators.SSC) || !params$stocks, results = 'asis'}
#This will only evaluate if there is no bulk density or if the user opted not to show stocks in the report
