...
title: |
  ...
  Land Steward Report
author: "Avalon Cook, Lisa Eash, Greg Richardson, Cathryn Mong, and Erika Foster"
...
raca_data<-reg_baseline(border)[[1]]%>% #use Lisa's function to get raca data for our region and extract the dataframe
  #select(upedonid, SOCstock30, LU)%>% #select relevant columns
  mutate(
    SOCstock30 = SOCstock30*0.4461, #convert metric to imperial
    LU = case_when( #adding this variable allows for binding to main dataset and faceting the graph below
      LU == "R" ~ "range",
      LU == "C" ~ "crop"
    ),
    plot_type = 'raca',
    Stocks_Indicator = "org_c_stocks")%>% #adding this helps later behavior when I filter based on carbon pool 
  select(upedonid, SOCstock30, plot_type, Stocks_Indicator, LU)
...
```
```{r results='asis', eval = unique(stocks_df$Stocks_Indicator)>1}
...
"The following graph shows **all other pools of carbon** measured in this project compared to a final value showing the sum of all average stocks pools in the study area."
)
```
```{r, fig.cap="Average carbon stocks for all measured pools. Organic carbon stocks are repeated as above to show comparison to other carbon pools measured. The values shown for all measured carbon pools combined is also shown."}
...
```{r Dynamic map}
#Start by creating the leaflet map for the plot borders

  pal <- colorFactor(c("white","black"), c("T", "C")) #define color palette for plot borders
  
  DynamicMap<-leaflet()%>% 
    #establishing the basemap first
    addProviderTiles(providers$Esri.WorldImagery) %>%
    #adding the border layer
    addPolygons(data = st_zm(border),
            fillOpacity = 0,
            color = ~ pal(plot_type),
            opacity=1)%>%
...
#Gather the names of the stocks columns
if ("bulk_density" %in% proj.indicators.SSC.stocks){
stockscolumns<-grep("_stocks$", proj.indicators.SSC.stocks, value = TRUE) 
}

#Gather all indicators except biomass -- this will be important later when we need to accomodate depth increments
proj.indicators.nonbiomass<-proj.indicators.SSC.stocks[!proj.indicators.SSC.stocks %in% c("AHB", "HRB", "AWB", "WRB")]

#Gather up all indicators that should be included in the map
if ("bulk_density" %in% proj.indicators.SSC.stocks) { 
...
#define an order for the relevant layers to appear on the map
...
#define rainbow colors for the map layers -- one color per value in dynamicmap.inds
base_colors <- rainbow(length(dynamicmap.inds))

#Prep labels for legend and popups using one of the dfs we made in the summary table process
MapLegendRef<-Means.Pivot1%>%
  mutate(legend = paste0(Acronym,ifelse(is.na(Units), "", paste0(" ", Units))))
legends_names <- setNames(MapLegendRef$legend, MapLegendRef$Indicator)
legends_names["sample_id"]<-"Sample ID" #forcing in value for sample ID

#####################This is where things need to loop according to timepoints#####################
#Prep main dataframe for map
DataMap.proj<-PointLevel%>%
  #Grab relevant columns from PointLevel
  ...
  by="sample_id")%>%
  #Ensure we're only looking at the first available timepoint
  arrange(timepoint)%>%
  filter(timepoint == first(timepoint))%>%
  st_as_sf()

#create dataframe for point popups
  #I used chat gpt for a lot of this cause I couldn't figure how to generate the tables row-wise so we're not stuck with a ton of NAs
  ...
  #Define helper function to build HTML for a single row (from ChatGPT)
  ...
  #Create a vector of column names for the popup
  popupcols<-setdiff(names(df_forpopup), c("sample_id", "geometry", "plot_type", "plot_type")) #define column to be included in the popups
  
  # Apply the function row-wise to create the popup table in raw HTML formatting
  df_forpopup$popup_html <- apply(
    df_forpopup, 1, 
    function(r) build_popup_html(r, popupcols)
  )

#To add points related to different indicators dynamically, this loop creates a layer for every value in dynamicmap.inds
overlay_groups <- c() #create empty vector for loop
#seq_along(dynamicmap.inds)[i]
for (i in seq_along(dynamicmap.inds)) {
  
  #Define the layer / legend name based on the named vector created above
  legend_name <- ifelse(dynamicmap.inds[i] %in% names(legends_names), 
                        legends_names[dynamicmap.inds[i]], 
                        dynamicmap.inds[i]) #if a readable legend name wasn't created, just use the raw column name
  
  #this prevents errors from being thrown in the next step when there are NAs 
  if (all(is.na(DataMap.proj[[dynamicmap.inds[i]]]))){domainz=c(0,1)
  }else{domainz=DataMap.proj[[dynamicmap.inds[i]]]}
    
  #establish color ramp for the current indicator based on the current rainbow base color
  pal<- colorNumeric(palette = colorRampPalette(c("white", base_colors[i]))(100), domain =domainz)

  #pipe the current indicator's point map into the plot border map
   ...
   overlay_groups <- c(overlay_groups, legend_name) #append the legend layer to the vector defining groups
  } #end loop creating layers
...
DynamicMap %>%
  ...
  )%>%hideGroup(setdiff(overlay_groups, "SOC %")) #hides all but the first layer to start
